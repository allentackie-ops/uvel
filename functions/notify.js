async function sendExpoPush(token, title, body, data = {}) {
  ...
  const channel = kind === "friend_message" || kind === "listing_message" || kind === "friend_request" || kind === "friend_accepted" ? "social" : kind === "sold" || kind === "shipped" || kind === "delivered" || kind === "wallet" ? "orders" : "activity";
  try {
    await fetch("https://exp.host/--/api/v2/push/send", {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ to: token, title, body, sound: "default", channelId: channel, priority: "high", data }),
    });
  } catch {
