...
exports.createCheckout = onCall({ secrets: [stripeSecret, paystackSecret] }, async (req) => {
  ...
  const { amountCents, currency, email, method, country, reference, name, orderId, listingId, brandId, variantKey, campaignId, collectionId, promotionId, promotionCode } = req.data || {};
  ...
  const normalizedPromotionCode = String(promotionCode || "").trim().toUpperCase().replace(/[^A-Z0-9_-]/g, "").slice(0, 32);
  ...
  if (String(order.currency || "").toUpperCase() !== normalizedCurrency) {
    throw new HttpsError("invalid-argument", "Order currency changed.");
  }
  ...
  const promotionQuote = (normalizedPromotionId || normalizedPromotionCode)
    ? await resolvePromotionQuote(admin.firestore(), { brandId: order.brandId || normalizedBrandId, listingId: String(order.pieceId || normalizedListingId), promotionId: normalizedPromotionId, code: normalizedPromotionCode, currency: normalizedCurrency, itemCents: Number(order.itemCents || 0) })
    : null;
  if ((normalizedPromotionId || normalizedPromotionCode) && !promotionQuote) {
...
exports.saveBrandCampaign = onCall((req) => saveMarketingRecord(req, "brandCampaigns", "campaign", "campaign_saved", req.data || {}, async (db, brandId, id, input) => {
  ...
  if (promotionId) {
    ...
    if (!promotionSnap.exists || promotionSnap.data()?.brandId !== brandId) throw new HttpsError("invalid-argument", "Choose a promotion owned by this brand.");
  }
  const requestedStatus = MARKETING_STATUSES.has(String(input.status || "draft")) ? String(input.status || "draft") : "draft";
  const status = requestedStatus === "live" && startAt && startAt > Date.now() ? "scheduled" : requestedStatus;
...
exports.reviewPhoto = onCall({ secrets: [anthropicSecret], timeoutSeconds: 60 }, async (req) => {
  if (!req.auth) throw new HttpsError("unauthenticated", "Sign in first.");
  const input = req.data || {};
  const mode = input.mode === "founder" ? "founder" : input.mode === "listing" ? "listing" : "";
  const mimeType = ["image/jpeg", "image/png", "image/webp"].includes(String(input.mimeType)) ? String(input.mimeType) : "image/jpeg";
  const base64 = String(input.base64 || "");
  if (!mode || !base64 || base64.length > 12 * 1024 * 1024) throw new HttpsError("invalid-argument", "That photo could not be read. Choose another image.");
  const key = anthropicSecret.value();
  if (!key) throw new HttpsError("failed-precondition", "Photo checking is not connected yet.");

  const listingSchema = "{\"ok\":boolean,\"score\":number,\"issues\":string[],\"tip\":string,\"title\":string,\"brand\":string,\"category\":string,\"color\":string,\"conditionGuess\":string,\"material\":string,\"description\":string}";
  const founderSchema = "{\"ok\":boolean,\"headline\":string,\"reasons\":string[]}";
  const prompt = mode === "founder"
    ? "You are Uvel's Founder Studio photo gate. The uploaded image is the proposed first fashion piece. Approve a clearly identifiable wearable item or fashion design reference: a garment, shoes, bag, jewelry, hat, scarf, belt, fashion illustration, line drawing, silhouette, or stylized monochrome design. A dark background, typography, logo, or abstract styling is not by itself a failure. Reject only unsafe or irrelevant content, an ordinary app/web screenshot or meme, a random graphic with no identifiable fashion purpose, or an image too blurry/dark/cropped to judge. Return ONLY JSON matching " + founderSchema + ". If it passes, headline must be \"This is the piece.\" and reasons must be []."
    : "You are Uvel's listing photo editor. Approve wearable fashion and clearly identifiable fashion sketches/design references. A dark background, typography, logo, editorial styling, or monochrome design is acceptable when it is relevant to a wearable item. Reject unsafe content, ordinary screenshots/memes/receipts, unrelated graphics, or images too blurry/dark/cropped to judge. Return ONLY JSON matching " + listingSchema + ".";
  let response;
  try {
    response = await fetch("https://api.anthropic.com/v1/messages", {
      method: "POST",
      headers: { "x-api-key": key, "anthropic-version": "2023-06-01", "content-type": "application/json" },
      body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: mode === "founder" ? 300 : 500, messages: [{ role: "user", content: [
        { type: "image", source: { type: "base64", media_type: mimeType, data: base64 } },
        { type: "text", text: prompt },
      ] }] }),
    });
  } catch {
    throw new HttpsError("unavailable", "Photo checking is temporarily unavailable. Try again in a moment.");
  }
  const payload = await response.json();
  if (!response.ok) throw new HttpsError("failed-precondition", payload?.error?.message || "Photo checking could not be completed.");
  const raw = String(payload?.content?.[0]?.text || "{}").trim();
  const start = raw.indexOf("{");
  const end = raw.lastIndexOf("}");
  if (start < 0 || end < start) throw new HttpsError("internal", "Photo checking returned an unreadable result.");
  let result;
  try { result = JSON.parse(raw.slice(start, end + 1)); } catch { throw new HttpsError("internal", "Photo checking returned an unreadable result."); }
  if (mode === "founder") return { ok: result.ok === true, headline: String(result.headline || (result.ok === true ? "This is the piece." : "That isn't the piece.")), reasons: Array.isArray(result.reasons) ? result.reasons.map(String).filter(Boolean).slice(0, 2) : [] };
  return { ok: result.ok === true, score: Math.max(1, Math.min(10, Number(result.score) || (result.ok === true ? 7 : 3))), issues: Array.isArray(result.issues) ? result.issues.map(String).filter(Boolean).slice(0, 2) : [], tip: String(result.tip || ""), title: String(result.title || ""), brand: String(result.brand || ""), category: String(result.category || "Tops"), color: String(result.color || ""), conditionGuess: String(result.conditionGuess || "Excellent"), material: String(result.material || ""), description: String(result.description || ""), analysisStatus: "complete" };
});
