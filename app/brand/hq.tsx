...
function MarketingSection({ brand, pieces, state, viewer, manager, theme, styles, onFocus }: { brand: Brand; pieces: ClosetPiece[]; state: MarketingState; viewer: boolean; manager: boolean; theme: HQTheme; styles: ReturnType<typeof make>; onFocus: () => void }) {
  ...
  const resetForm = () => { setCollectionName(""); setCollectionDescription(""); setCampaignName(""); setCampaignHeadline(""); setCampaignBody(""); setCampaignPromotionId(""); setPromotionCode(""); setPromotionValue(""); setMinimumOrder(""); setPromotionUsageLimit(""); setStartDate(""); setEndDate(""); setSelectedProductIds([]); setLaunchStatus("draft"); };
  ...
  async function save() {
    ...
    try {
      ...
      if (tab === "collections") {
        ...
        await saveBrandCampaign({ brandId: brand.id, name: campaignName, headline: campaignHeadline, body: campaignBody, channel: campaignChannel, promotionId: campaignPromotionId || undefined, productIds: selectedProductIds, status: statusForSave(times.startAt), ...times });
      } else {
