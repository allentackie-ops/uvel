...
type OrderFilter = "all" | "active" | "attention" | "action" | "processing" | "packed" | "shipped" | "completed" | "canceled";
...
const UVEL_ORDER_FILTERS: Array<{ id: OrderFilter; label: string }> = [
  { id: "all", label: "All" },
...
function orderNeedsAttention(order: Order) {
  return order.status === "failed" || order.shipment?.status === "exception" || Boolean(order.resolution && ["requested", "approved", "item_sent", "received", "refund_pending"].includes(order.resolution.status));
}

function OrdersSection({ orders, madeByUvel, viewer, manager, reviewer, onSupport, theme, styles }: { orders: Order[]; madeByUvel?: boolean; viewer: boolean; manager: boolean; reviewer: boolean; onSupport: () => void; theme: HQTheme; styles: ReturnType<typeof make> }) {
  ...
  const activeOrders = orders.filter((order) => order.status === "paid" && !["delivered", "canceled", "returned"].includes(order.fulfillmentStatus || "unfulfilled"));
  const attentionOrders = orders.filter(orderNeedsAttention);
  const filters = madeByUvel ? UVEL_ORDER_FILTERS : ORDER_FILTERS;
  const filtered = orders.filter((order) => {
    const status = order.fulfillmentStatus || (order.status === "paid" ? "unfulfilled" : undefined);
    if (filter === "all") return true;
    if (filter === "active") return activeOrders.includes(order);
    if (filter === "attention") return attentionOrders.includes(order);
    ...
    <View>
      ...
      {madeByUvel ? <>
        ...
        <View style={styles.orderStats}>
          ...
          <Stat label="Needs attention" value={String(attentionOrders.length)} theme={theme} styles={styles} />
        </View>
      </> : <>
      ...
      {filtered.length ? filtered.map((order) => <OrderCard key={order.id} order={order} madeByUvel={Boolean(madeByUvel || order.madeByUvel)} manager={manager} reviewer={reviewer} onSupport={onSupport} theme={theme} styles={styles} />) : <Empty text={orders.length ? "No orders match this filter." : "No brand orders yet."} theme={theme} styles={styles} />}
    </View>
  );
}
...
function OrderCard({ order, madeByUvel, manager, reviewer, onSupport, theme, styles }: { order: Order; madeByUvel?: boolean; manager: boolean; reviewer: boolean; onSupport: () => void; theme: HQTheme; styles: ReturnType<typeof make> }) {
    ...
    <View>
      ...
      <Sheet open={showReviewForm} onClose={() => setShowReviewForm(false)} expandable surfaceColor={theme.card}>
        <ScrollView style={styles.reviewSheetScroll} contentContainerStyle={styles.reviewSheetContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          <TextInput editable={owner} value={provider} onChangeText={setProvider} placeholder="Registration source (optional)" placeholderTextColor={theme.muted} style={[styles.registrationInput, { color: theme.ink, borderColor: theme.lineColor }]} />
          <TextInput editable={owner} value={legalName} onChangeText={setLegalName} placeholder="Legal/business name" placeholderTextColor={theme.muted} style={[styles.registrationInput, { color: theme.ink, borderColor: theme.lineColor }]} />
          <TextInput editable={owner} value={registrationId} onChangeText={setRegistrationId} placeholder="Registration number" placeholderTextColor={theme.muted} style={[styles.registrationInput, { color: theme.ink, borderColor: theme.lineColor }]} />
          <View style={styles.attachmentActions}><Pressable onPress={() => void chooseFile()} style={[styles.attachmentButton, { borderColor: theme.lineColor }]}><Text style={[styles.proofButtonText, { color: theme.ink }]}>Attach PDF or file</Text></Pressable><Pressable onPress={() => void choosePhoto()} style={[styles.attachmentButton, { borderColor: theme.lineColor }]}><Text style={[styles.proofButtonText, { color: theme.ink }]}>Choose photo</Text></Pressable></View>
          {proofName ? <View style={styles.attachmentName}><Text style={[styles.proofButtonText, { color: theme.ink }]} numberOfLines={1}>{proofName}</Text><Pressable onPress={() => { setProofUri(""); setProofName(""); }}><Text style={[styles.removeProof, { color: theme.muted }]}>Remove</Text></Pressable></View> : null}
          <Text style={[styles.registrationHelp, { color: theme.muted }]}>Uvel will review the document before showing Verified.</Text>
          <View style={styles.reviewSheetActions}>
            <Pressable onPress={() => setShowReviewForm(false)} style={[styles.actionButton, styles.reviewSheetActionButton, { borderColor: theme.lineColor }]} accessibilityRole="button"><Text style={[styles.actionButtonTxt, { color: theme.ink }]}>Cancel</Text></Pressable>
            <Pressable disabled={busy} onPress={() => void submit()} style={[styles.saveButton, styles.reviewSheetActionButton, { backgroundColor: theme.accent, opacity: busy ? 0.5 : 1 }]} accessibilityRole="button"><Text style={[styles.saveButtonTxt, { color: theme.accentInk }]}>{busy ? "Submitting…" : "Submit for review"}</Text></Pressable>
          </View>
        </ScrollView>
      </Sheet>
    </View>
